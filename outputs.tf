...
output "rsync_command" {
  ...
  value       = "rsync -avz --progress --exclude='.next/' --exclude='node_modules/' --exclude='.git/' -e 'ssh -i ${var.create_key_pair ? "${var.key_name}.pem" : var.private_key_path}' ./ ubuntu@${aws_eip.web_eip.public_ip}:/home/ubuntu/quran-with-maryam-web-client"
}